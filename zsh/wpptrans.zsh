...
_wpptrans_decode_path() {
    ...
    raw="${raw#file://}"
    # Decodifica URL encoding (%20 etc.) via python
    python -c "import sys, urllib.parse; print(urllib.parse.unquote(sys.argv[1]))" "$raw"
}
...
_wpptrans_run() {
    ...
    # Converte ogg/opus para wav 16kHz mono (formato esperado pelo whisper)
...
wpptrans_rm() {
    ...
    # Segurança: só remove arquivos dentro de ~/Downloads/
    local downloads_dir
